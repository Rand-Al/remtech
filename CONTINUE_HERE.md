...
- Пользователь выбрал следующий шаг: завершить Этап 1 подключением реальной локальной LLM через LM Studio, сохранив универсальную архитектуру провайдеров. LM Studio должен быть первой конфигурацией в общем реестре, а не жестко заданным особым случаем.
...
- Единый каталог цен находится в `web/shared/pricing.ts`. Его используют компоненты сайта и чат-сервер; тестовые цены котлов больше не дублируются в разметке или ответах чата.
...
- клиент может приложить до трёх фотографий к одному сообщению: JPEG, PNG, WebP, HEIC или HEIF, до 10 МБ каждая;
- перед отправкой показывается предпросмотр, выбранное фото можно удалить;
- файлы сохраняются чат-сервером в `RT_UPLOAD_DIR` (`data/uploads` по умолчанию), а связь с заявкой и сообщением хранится в PostgreSQL;
- ссылки на фотографии доступны только с ключом соответствующего чата, а в уведомление мастеру передаётся количество вложений.
...
Перед реальным подключением нужно заменить выбор адаптера из переменных окружения на общий реестр провайдеров. Конфигурация хранится в PostgreSQL, а будущая админка позволяет добавлять подключения, выбирать основной и резервный провайдеры, менять модель и проверять соединение без правки кода и перезапуска сайта. API-ключи сохраняются только в зашифрованном виде.
